...
export function FAQs({ _key, title, faqs }: FAQsProps) {
    ...
    return (
        <section className="container mx-auto flex flex-col gap-8 py-16">
            ...
            {title ? (
                <h2 className="text-xl mx-auto md:text-2xl lg:text-5xl font-semibold text-slate-800 text-pretty max-w-3xl">{title}</h2>
            ) : null}
            {Array.isArray(faqs) ? (
                <div className="max-w-2xl mx-auto border-b border-pink-200">
                    {faqs.map((faq) => (
                        ...
                        >
                            ...
                            <div className="pb-4">{faq.body ? <PortableText value={faq.body} /> : null}</div>
                        </details>
                    ))}
                </div>
            ) : null}
